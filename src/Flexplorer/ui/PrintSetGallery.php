...
class PrintSetGallery extends \Ease\TWB5\Tabs
{
    ...
    public function __construct($engine, $mode = 'single')
    {
        ...
        if (empty($printSets)) {
            $this->addTab(_('none'), new \Ease\Html\DivTag(new \Ease\TWB5\Alert('warning', _('No print reports available for this evidence.'))));
        ...
        if (empty($filteredReports)) {
            ...
            $this->addTab(
                _('Info'),
                new \Ease\Html\DivTag(new \Ease\TWB5\Alert('info', $message)),
            );
