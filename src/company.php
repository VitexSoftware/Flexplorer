...
if (empty($company)) {
    ...
    $companyActions->addColumn(2,
            new \Ease\TWB5\LinkButton('resetcompany.php',
                    new \Ease\TWB5\GlyphIcon('repeat') . ' ' . _('Reset'), 'danger',
                    ['onClick' => "$('#Preloader').css('visibility', 'visible');", 'title' => _('Drop company and create again')]));

    $companyActions->addColumn(1,
            new \Ease\TWB5\LinkButton('copycompany.php',
                    new \Ease\TWB5\GlyphIcon('duplicate') . ' ' . _('Duplicate'), 'success',
                    ['onClick' => "$('#Preloader').css('visibility', 'visible');", 'title' => _('Create copy')]));

    $backupFile = '../backups/' . $company . '.winstrom-backup';

    $companyActions->addColumn(2,
            new \Ease\TWB5\LinkButton('restorecompany.php',
                    new \Ease\TWB5\GlyphIcon('floppy-open') . ' ' . _('Restore'), 'warning',
                    ['onClick' => "$('#Preloader').css('visibility', 'visible');", 'title' => _('Restore previously saved state')]));

    $companyActions->addColumn(1,
            new \Ease\TWB5\LinkButton('savecompany.php',
                    new \Ease\TWB5\GlyphIcon('floppy-save') . ' ' . _('Save'), 'success',
                    ['title' => _('Save current state')]));

    $companyActions->addColumn(2,
            new \Ease\TWB5\LinkButton('deletecompany.php',
                    new \Ease\TWB5\GlyphIcon('remove') . ' ' . _('Remove'), 'danger',
                    ['onClick' => "$('#Preloader').css('visibility', 'visible');", 'title' => 'Drop all company data']));

    $companyActions->addColumn(2,
            new \Ease\TWB5\LinkButton('editor.php?evidence=nastaveni&company=' . $company . '&id=1',
                    new \Ease\TWB5\GlyphIcon('wrench') . ' ' . _('Settings'), 'info',
                    ['title' => 'Serveral company settings']));

    $companyActions->addColumn(2,
            new \Ease\TWB5\LinkButton('newcompany.php',
                    new \Ease\TWB5\GlyphIcon('plus') . ' ' . _('Create company'), 'success',
    ...
    $companyInfo->addRowColumns([_('created'), strftime('%a %d. %m. %Y  - %X',
                $created) . ' ' . '(' . _('before') . ' ' . new \Ease\TWB5\Widgets\LiveAge($created) . ')']);
    ...
    $companyInfo->addRowColumns([_('Status'), new \Ease\TWB5\Label($companer->getDataValue('stavEnum') == 'ESTABLISHED' ? 'success' : 'warning',
                $companer->getDataValue('stavEnum'))]);

    $companyPanel = new \Ease\TWB5\Panel(new \Ease\Html\H2Tag($companer->getDataValue('nazev')),
